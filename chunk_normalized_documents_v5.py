...
# Number of COMPLETE data rows repeated between consecutive
# table chunks. This is row overlap, not character overlap.
TABLE_ROW_OVERLAP = 1
...
def chunk_table_rows(
    ...
    row_overlap: int = TABLE_ROW_OVERLAP
) -> List[str]:
    """
    ...
    - repeat table header in every chunk
    - overlap exactly `row_overlap` COMPLETE data rows
    - never use character overlap inside a table row

    With TABLE_ROW_OVERLAP = 1:

        Chunk 1: header + rows 1-10
        Chunk 2: header + row 10 + rows 11-19
        Chunk 3: header + row 19 + rows 20-28

    If the overlap row plus the next row would make a new chunk
    exceed max_chars immediately, the overlap is dropped for that
    boundary rather than splitting a row.
    """
    ...
    if row_overlap < 0:
        raise ValueError(
            "TABLE_ROW_OVERLAP cannot be negative."
        )
    ...
    for row in data_rows:
        ...
        # Add the full row if the current chunk still fits.
        ...
        # Carry complete overlap row(s) into the next chunk.
        if (
            row_overlap > 0
            and current_rows
        ):
            overlap_rows = current_rows[
                -row_overlap:
            ]
        else:
            overlap_rows = []
        ...
        # If overlap + the new row already exceeds the target,
        # discard the overlap for this boundary. The new row is
        # still preserved whole.
        overlap_candidate = (
            f"Table: {table_name}\n\n"
            + rows_to_markdown(
                [header]
        ...
        if (
            len(overlap_candidate) > max_chars
            and overlap_rows
        ):
            current_rows = [row]

    # Save final table chunk.
    if current_rows:
        chunks.append(
            (
                f"Table: {table_name}\n\n"
